...
const apiModel = "gpt-4.1-nano"
const openAIService = new OpenAIService(apiModel, systemMessage)

async function startDialog() {
    const history: ContextAPIProps[] = [];
    const initialMessage = "Ah, apareceu um negócio estranho aqui na minha tela. Como faço pra ver aquele vídeo mesmo?";
    history.push({ role: "user", content: initialMessage });

    await openAIService.sendMessage({ textStream: initialMessage });
